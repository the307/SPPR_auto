...
def main():
    ...
    # --- LODOCHNY ---
    Q_tagulsk_prev = master_df.loc[master_df["date"].dt.month == prev_month.month, "gtm_tagulsk"].values
    G_lodochni_upsv_yu_prev = master_df.loc[master_df["date"].dt.month == prev_month.month, "lodochni_upsv_yu"].values
    ...
    K_gupn_lodochny = float(input("Введите K_GУПН_Лодочный: "))
    ...
    lodochny_results = calculate.lodochny(
        ...
        Q_tagul_prev=Q_tagulsk_prev,
        G_lodochni_upsv_yu_prev=G_lodochni_upsv_yu_prev,
