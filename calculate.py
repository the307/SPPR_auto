...
# 37. Расчет наличия нефти в РВС ЦПС, т:
    ...
    if not flag_list[2]:
        if not (V_upsv_cps_prev - 1500 <= V_upsv_cps <= V_upsv_cps_prev + 1500):
            raise CalculationValidationError(
                f"УПСВ-ЦПС: уровень вне допустимой вилки ±1500 т "
                f"(текущее {V_upsv_cps:.2f}, вчера {V_upsv_cps_prev:.2f})"
            )
    else:
        if not (V_upsv_cps_prev - 2000 <= V_upsv_cps <= V_upsv_cps_prev + 3300):
            raise CalculationValidationError(
...
# ===============================================================
def rn_vankor(
    F_vn, F_suzun_obsh, F_suzun_vankor, N, day,V_ctn_suzun_vslu_norm, V_ctn_suzun_vslu,
...
# ===============================================================
def sikn_1208 (
    G_suzun_vslu, G_sikn_tagul_lod_data, G_buy_day, G_per, G_suzun, G_sikn_tagul, G_suzun_tng, Q_vankor, V_upsv_yu, 
...
# 60.	Расчет потерь при откачке через СИКН № 1208 (потери+отпуск+прочее), т/сут:
    ...
    G_delta_sikn_month = sum(G_delta_sikn_month) # сумма на текущий день месяца
    ...
    return {
        ...
        "G_sikn_vankor":G_sikn_vankor, "G_sikn_vankor_month":G_sikn_vankor_month, "G_skn_month":G_skn_month, "G_delta_sikn":G_delta_sikn," G_delta_sikn_month": G_delta_sikn_month,
    }

def TSTN (
        V_gnsp_0,V_gnsp_prev, N, VN_min_gnsp, G_sikn, flag_list, V_nps_1_prev, V_nps_2_prev, G_tagul, G_upn_lodochny, G_skn, G_kchng,
        ...
        G_sikn_tagul, F_tagul_lpu, K_lodochny, V_tstn_rn_vn_prev, manual_G_gnsp_i, G_lodochny, V_tsnt_suzun_vankor_0,
        V_tstn_vslu_0, V_tstn_tagul_0, V_tstn_lodochny_0, V_tstn_rn_vn_0, V_tstn_suzun_vankor_0, V_tstn_suzun_vslu_0,
        V_tstn_kchng_0, V_tstn_skn_0, V_tstn_vo_0, V_tstn_tng_0, 
          ):
    # Массивы для расчета месячных значений
    G_gpns_data = []
    # ---------- Инициализация-------------
...
# 61.	Расчет откачки нефти с ГНПС, т/сут:
    if manual_G_gnsp_i is not None:
        G_gpns_i = manual_G_gnsp_i
    else:
        G_gpns_i = G_sikn + (V_gnsp_0 - VN_min_gnsp)/N
    G_gpns_data.append(G_gpns_i)
    G_gpns_month = sum(G_gpns_data)
...
# 62.	Расчет наличия нефти в РП ГНПС, т:
    ...
    V_gnsp_out = V_gnsp
    if not flag_list[0]:
        if not (V_gnsp_prev-1500 <= V_gnsp <= V_gnsp_prev+1500):
            V_gnsp_out = {
                "value": V_gnsp,
                "status": 1,
                "message": (
                    f"РП ГНПС: уровень вне допустимой вилки ±1500 т "
                    f"(текущее {V_gnsp:.2f}, вчера {V_gnsp_prev:.2f})"
                ),
            }
    else:
        if not (V_gnsp_prev - 2000 <= V_gnsp <= V_gnsp_prev + 3000):
            V_gnsp_out = {
                "value": V_gnsp,
                "status": 1,
                "message": (
...
#65.	Расчет наличия нефти в РП КНПС
    ...
    V_knsp_out = V_knsp
    if not flag_list[2]:
        if not (V_knps_prev - 1500 <= V_knsp <= V_knps_prev + 1500):
            V_knsp_out = {
                "value":V_knsp,
                "status":1,
                "message":(
                f"РП КНПС: уровень вне допустимой вилки ±1500 т "
                f"(текущее {V_knsp:.2f}, вчера {V_knps_prev:.2f})"),
            }
    else:
        if not (V_knps_prev - 2000 <= V_knsp <= V_knps_prev + 3000):
            V_knsp_out = {
                "value":V_knsp,
                "status":1,
                "message":(
...
# 78.	Расчет наличия нефти АО «Ванкорнефть» в резервуарах ЦТН, т:
    ...
    return {
        "G_gpns_i":G_gpns_i, "G_gpns_month":G_gpns_month, "G_gpns":G_gpns, "V_gnsp":V_gnsp_out, "V_nps_1":V_nps_1_out, "V_nps_2":V_nps_2,"V_nps_2_out":V_nps_2_out, "V_knsp":V_knsp,
        ...
        "V_tstn_suzun":V_tstn_suzun_out, "V_tstn_skn":V_tstn_skn_out,"V_tstn_vo":V_tstn_vo, "V_tstn_tng":V_tstn_tng_out, "V_tstn_kchng":V_tstn_kchng_out, "V_knsp_out":V_knsp_out,
        "V_tstn_tagul":V_tstn_tagul_out, "V_tstn_lodochny":V_tstn_lodochny_out, "V_tstn_tagul_obsh":V_tstn_tagul_obsh, "V_tstn_tagul_obsh_0":V_tstn_tagul_obsh_0,
        "V_tstn_rn_vn":V_tstn_rn_vn, "V_tstn_vn":V_tstn_vn_out, "V_tstn_vn_0":V_tstn_vn_0_out
    }
# ===============================================================
...
# --- 87. Проверка выполнения условий по наличию нефти на УПН Лодочное
    ...
    if delta_upn_lodochny >= 0:
        ...
        if abs(delta_upn_lodochny) > VO_delta_upn_lodochny_max:
...
# --- 88. Проверка выполнения условий по наличию нефти на Тагульском месторождении
    V_tagul_tr_out = {"value": V_tagul_tr, "status": 0, "message": ""}
    if VA_tagul_tr_min <= V_tagul_tr <= VA_tagul_tr_max:
        V_tagul_tr_out.update({"status": 1, "message": "Проверка выполнения условий по наличию нефти на Тагульском месторождении"})
    else:
        if V_tagul_tr < VA_tagul_tr_min:
            ...
            V_tagul_tr_out.update({"status": 2, "message": msg})
    ...
    if delta_gnps >= 0:
        ...
        if abs(delta_gnps) > VO_delta_gnps_max:
...
# --- 90. Проверка выполнения условий по наличию нефти на НПС-1
    ...
    if delta_nps_1 >= 0:
        ...
        if abs(delta_nps_1) > VO_delta_nps_1_max:
...
# --- 91. Проверка выполнения условий по наличию нефти на НПС-2
    ...
    if delta_nps_2 >= 0:
        ...
        if abs(delta_nps_2) > VO_delta_nps_2_max:
...
# --- 92. Проверка выполнения условий по наличию нефти на КНПС
    ...
    if VA_knps_min <= V_knps <= VA_knps_max:
    ...
    if delta_knps >= 0:
        ...
        if abs(delta_knps) > VO_delta_knps_max:
...
# --- 95.	Проверка наличия нефти на объектах ЦТН по недропользователям
    V_ctn_vn_out = {"value": V_ctn_vn, "status": 0, "message": ""}
    if V_ctn_vn_min <= V_ctn_vn <= V_ctn_vn_max:
        V_ctn_vn_out.update({"status": 1, "message": "Проверка выполнения условий по наличию нефти АО «Ванкорнефть» на ЦТН"})
    else:
        if V_ctn_vn < V_ctn_vn_min:
            ...
            V_ctn_vn_out.update({"status": 2, "message": msg})
            ...
            V_ctn_vn_out.update({"status": 3, "message": msg})

    V_ctn_suzun_out = {"value": V_ctn_suzun, "status": 0, "message": ""}
    if V_ctn_suzun_min <= V_ctn_suzun <= V_ctn_suzun_max:
        V_ctn_suzun_out.update({"status": 1, "message": "Проверка выполнения условий по наличию нефти АО «Сузун» на ЦТН"})
    else:
        if V_ctn_suzun < V_ctn_suzun_min:
            ...
            V_ctn_suzun_out.update({"status": 2, "message": msg})
            ...
            V_ctn_suzun_out.update({"status": 3, "message": msg})

    V_ctn_suzun_vankor_out = {"value": V_ctn_suzun_vankor, "status": 0, "message": ""}
    if V_ctn_suzun_vankor_min <= V_ctn_suzun_vankor <= V_ctn_suzun_vankor_max:
        V_ctn_suzun_vankor_out.update({"status": 1, "message": "Проверка выполнения условий по наличию нефти АО «Сузун» (Ванкор) на ЦТН"})
    else:
        if V_ctn_suzun_vankor < V_ctn_suzun_vankor_min:
            ...
            V_ctn_suzun_vankor_out.update({"status": 2, "message": msg})
            ...
            V_ctn_suzun_vankor_out.update({"status": 3, "message": msg})

    V_ctn_suzun_vsly_out = {"value": V_ctn_suzun_vsly, "status": 0, "message": ""}
    if V_ctn_suzun_vsly_min <= V_ctn_suzun_vsly <= V_ctn_suzun_vsly_max:
        V_ctn_suzun_vsly_out.update({"status": 1, "message": "Проверка выполнения условий по наличию нефти АО «Сузун» (ВСЛУ) на ЦТН"})
    else:
        if V_ctn_suzun_vsly < V_ctn_suzun_vsly_min:
            ...
            V_ctn_suzun_vsly_out.update({"status": 2, "message": msg})
            ...
            V_ctn_suzun_vsly_out.update({"status": 3, "message": msg})
    
    V_ctn_tagul_obch_out = {"value": V_ctn_tagul_obch, "status": 0, "message": ""}
    if V_ctn_tagul_obch_min <= V_ctn_tagul_obch <= V_ctn_tagul_obch_max:
        V_ctn_tagul_obch_out.update({"status": 1, "message": "Проверка выполнения условий по наличию нефти ООО «Тагульское» (всего) на ЦТН"})
    else:
        if V_ctn_tagul_obch < V_ctn_tagul_obch_min:
            ...
            V_ctn_tagul_obch_out.update({"status": 2, "message": msg})
            ...
            V_ctn_tagul_obch_out.update({"status": 3, "message": msg})
    
    V_ctn_lodochny_out = {"value": V_ctn_lodochny, "status": 0, "message": ""}
    if V_ctn_lodochny_min <= V_ctn_lodochny <= V_ctn_lodochny_max:
        V_ctn_lodochny_out.update({"status": 1, "message": "Проверка выполнения условий по наличию нефти ООО «Тагульское» (Лодочный ЛУ) на ЦТН"})
    else:
        if V_ctn_lodochny < V_ctn_lodochny_min:
            ...
            V_ctn_lodochny_out.update({"status": 2, "message": msg})
            ...
            V_ctn_lodochny_out.update({"status": 3, "message": msg})
    
    V_ctn_tagul_out = {"value": V_ctn_tagul, "status": 0, "message": ""}
    if V_ctn_tagul_min <= V_ctn_tagul <= V_ctn_tagul_max:
        V_ctn_tagul_out.update({"status": 1, "message": "Проверка выполнения условий по наличию нефти ООО «Тагульское» (Тагульский ЛУ) на ЦТН"})
    else:
        if V_ctn_tagul < V_ctn_tagul_min:
            ...
            V_ctn_tagul_out.update({"status": 2, "message": msg})
            ...
            V_ctn_tagul_out.update({"status": 3, "message": msg})
    
    V_ctn_skn_out = {"value": V_ctn_skn, "status": 0, "message": ""}
    if V_ctn_skn_min <= V_ctn_skn <= V_ctn_skn_max:
        V_ctn_skn_out.update({"status": 1, "message": "Проверка выполнения условий по наличию нефти ООО «СевКомНефтегаз» на ЦТН"})
    else:
        if V_ctn_skn < V_ctn_skn_min:
            ...
            V_ctn_skn_out.update({"status": 2, "message": msg})
            ...
            V_ctn_skn_out.update({"status": 3, "message": msg})
    
    V_ctn_vo_out = {"value": V_ctn_vo, "status": 0, "message": ""}
    if V_ctn_vo_min <= V_ctn_vo <= V_ctn_vo_max:
        V_ctn_vo_out.update({"status": 1, "message": "Проверка выполнения условий по наличию нефти ООО «Восток Ойл» (Ичемминский ЛУ) на ЦТН"})
    else:
        if V_ctn_vo < V_ctn_vo_min:
            ...
            V_ctn_vo_out.update({"status": 2, "message": msg})
            ...
            V_ctn_vo_out.update({"status": 3, "message": msg})
    
    V_ctn_tng_out = {"value": V_ctn_tng, "status": 0, "message": ""}
    if V_ctn_tng_min <= V_ctn_tng <= V_ctn_tng_max:
        V_ctn_tng_out.update({"status": 1, "message": "Проверка выполнения условий по наличию нефти АО «Таймырнефтегаз» (Пайяхский ЛУ) на ЦТН"})
    else:
        if V_ctn_tng < V_ctn_tng_min:
            ...
            V_ctn_tng_out.update({"status": 2, "message": msg})
            ...
            V_ctn_tng_out.update({"status": 3, "message": msg})
    
    V_ctn_kchng_out = {"value": V_ctn_kchng, "status": 0, "message": ""}
    if V_ctn_kchng_min <= V_ctn_kchng <= V_ctn_kchng_max:
        V_ctn_kchng_out.update({"status": 1, "message": "Проверка выполнения условий по наличию нефти ООО «КЧНГ» (Русско-Реченское месторождение) на ЦТН"})
    else:
        if V_ctn_kchng < V_ctn_kchng_min:
            ...
            V_ctn_kchng_out.update({"status": 2, "message": msg})
            ...
            V_ctn_kchng_out.update({"status": 3, "message": msg})
...
# --- 96. Проверка соблюдения нормативных значений насосного оборудования ГНПС
    ...
    return{
        ...
        "V_tagul_tr_out":V_tagul_tr_out, "V_gnps_out":V_gnps_out, "V_nps_1_check_out":V_nps_1_check_out, "V_nps_2_check_out":V_nps_2_check_out,
        "V_knps_check_out":V_knps_check_out, "V_ichem_out":V_ichem_out, "V_lodochny_cps_uspv_yu_out":V_lodochny_cps_uspv_yu_out, "V_ctn_vn_out":V_ctn_vn_out,
        "V_ctn_suzun_out":V_ctn_suzun_out, "V_ctn_suzun_vankor_out":V_ctn_suzun_vankor_out, "V_ctn_suzun_vsly_out":V_ctn_suzun_vsly_out,
        "V_ctn_tagul_obch_out":V_ctn_tagul_obch_out, "V_ctn_lodochny_out":V_ctn_lodochny_out, "V_ctn_tagul_out":V_ctn_tagul_out, 
    }   
# ===============================================================
...
def balance_po_business_plan (
        ...
        V_lodochny_oil, V_lodochny_transport, G_lodochny_fuel, G_lodochny_release_rn_drillig, V_lodochny_km_ost_np, V_lodochny_km_ost_app,
        V_lodochny_km_ost_texn, V_lodochny_km_path, F_lodochny_total, V_tagul_nm_ost_np, V_tagul_nm_ost_app, V_tagul_nm_ost_texn,
        V_tagul_nm_path, Q_tagul_oil, Q_tagul_condensate, V_tagul_oil, V_tagul_transport, G_tagul_fuel, G_tagul_release_rn_drillig,
...
# Технологические потери нефти (газового конденсата)
    V_tagul_lost = V_tagul_oil + V_tagul_transport
# Расход нефти (газового конденсата) на собственные производственно-технологические нужды и топливо
...
def plan_balance_gtm (
        V_cppn_1_0, V_vn_nm_gtm_ost_dead, V_ctn_vn_0, V_vn_nm_gtm_ost_np, V_vn_nm_gtm_ost_app, Q_vankor_month, Q_vn_gtm_condensate, F_vn_month,
        ...
        V_suzun_0, V_vankor_suzun_0, V_vankor_vslu_0, Q_suzun_month, Q_vslu_month, Q_suzun_gtm_conden, K_suzun_mining, G_per_month, G_suzun_gtm_fuel,
        ...
        V_suzun_vslu, V_suzun_km_gtm_ost_vps, G_suzun_gtm_release, V_tstn_suzun, V_tstn_suzun_vslu, V_ichem_0, V_vo_nm_gtm_ost_texn,
        V_tstn_vo_0, V_vo_nm_gtm_ost_np, V_vo_nm_gtm_app, Q_vo_month, Q_vo_gtm_conden, F_vo_delta, K_vo_trans, K_vo_moning, V_vo_gtm_lost_conden,
        ...
        Q_lodochny_month, Q_lodochny_gtm_conden, F_tagul_lpy_delta, R_lodochny_mining, K_lodochny_trans, V_lodochny_nm_gtm_dead, V_lodochny_nm_gtm_ost_product,
        ...
        V_tstn_tagul_0, Q_tagul_month, Q_tagul_gtm_conden, F_tagul_tpy_delta, K_tagul_minig, K_tagul, G_tagul_gtm_fuel, G_tagul_gtm_release_russko_rechenskoe,
...
# Добыча нефти (газового конденсата) АО «Сузун», всего
    Q_suzun_gtm_total = Q_suzun_gtm_oil_slu + Q_suzun_gtm_oil_vslu + Q_suzun_gtm_conden
# Технологические потери нефти при добыче и подготовке
...
# Сдача нефти АО «Сузун» (Сузунский ЛУ)
    F_suzun_gtm_slu = F_suzun_delta
# Сдача нефти АО «Сузун» (Восточно-Сузунский ЛУ)
    F_suzun_gtm_vslu = F_suzun_vsly_delta
# Сдача нефти АО «Сузун» (АО «Ванкорнефть»)
...
# Технологические остатки нефти СПУ на конец месяца отчетного периода
    V_suzun_km_gtm_ost_texn = V_suzun_km_gtm_ost_upn + V_suzun_km_gtm_ost_vps
...
# Остатки нефти (газового конденсата) на ВПУ
    V_vo_nm_gtm_ost_vpu = V_vo_nm_gtm_ost_np + V_vo_nm_gtm_app + V_vo_nm_gtm_ost_rvs
...
# Добыча нефти (газового конденсата) ООО «Восток Ойл», всего
    Q_vo_gtm_total = Q_vo_gtm_oil + Q_vo_gtm_conden
# Сдача нефти (газового конденсата) ООО «Восток Ойл», всего
    F_vo_gtm_total = F_vo_delta
# Технологические потери нефти при добыче и подготовке
    V_vo_gtm_lost_oil = round(Q_vo_gtm_oil * (K_vo_moning/100))
# Технологические потери нефти (газового конденсата) при транспортировке
    V_vo_gtm_lost_trans = round(F_vo_gtm_total * (K_vo_trans/100))
# Технологические потери нефти (газового конденсата), всего
    V_vo_gtm_lost = V_vo_gtm_lost_oil + V_vo_gtm_lost_conden + V_vo_gtm_lost_trans
# Расход нефти (газового конденсата) на собственные производственно-технологические нужды и топливо
...
# Выполнение процедуры проверки
    ...
    if V_vo_gtm_check != 0:
        ...
        if abs(V_tstn_vo - V_vo_km_gtm_path)<=40:
            print("Проверка пройдена")
        else:
            print("Проверка не пройдена. Необходимо уточнить корректность введенных данных")
...
# Добыча нефти (газового конденсата) ООО «Тагульское» Лодочное месторождение, всего
    Q_lodochny_gtm_total = Q_lodochny_gtm_oil + Q_lodochny_gtm_conden
# Сдача нефти (газового конденсата) ООО «Тагульское» Лодочное месторождение, всего
    F_lodochny_gtm_total = F_tagul_lpy_delta
# Технологические потери нефти при добыче и подготовке
    V_lodochny_gtm_lost_oil = round(Q_lodochny_gtm_oil * (R_lodochny_mining/100))
# Технологические потери нефти (газового конденсата) при транспортировке
...
# Выполнение процедуры проверки
    ...
    if V_lodochny_gtm_check != 0:
        print("Проверка не пройдена. Необходимо уточнить корректность введенных данных")
    else:
        if abs(V_tstn_lodochny - V_lodochny_km_gtm_path) <=5:
...
# Добыча нефти (газового конденсата) ООО «Тагульское» Тагульское месторождения, всего
    Q_tagul_gtm_total = Q_tagul_gtm_oil + Q_tagul_gtm_conden
...
# Выполнение процедуры проверки
    ...
    if V_tagul_gtm_check !=0:
        print("Проверка не пройдена. Необходимо уточнить корректность введенных данных")
    else:
        if abs(V_tstn_tagul - V_tagul_km_gtm_path) <=5:
            print("Проверка пройдена.")
        else:
            print("Проверка не пройдена. Необходимо уточнить корректность введенных данных")
